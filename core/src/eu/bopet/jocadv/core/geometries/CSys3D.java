...
public class CSys3D extends Feature implements Geometry {
    public static final CSys3D DEFAULT = new CSys3D(
            Vector3D.ORIGIN,
            Axis3D.X,Axis3D.Y,Axis3D.Z,
            Plane3D.YZ,Plane3D.XZ,Plane3D.XY);
    ...
    public CSys3D(Vector3D origin,
                  Axis3D xAxis, Axis3D yAxis, Axis3D zAxis,
                  Plane3D yzPlane, Plane3D xzPlane, Plane3D xyPlane) {
        this.origin = origin;
        this.xAxis = xAxis;
        this.yAxis = yAxis;
        this.zAxis = zAxis;
